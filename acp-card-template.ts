...
interface ThemeColors {
  ...
  bgAlt: string;        // 次级背景
  primary: string;      // 主色
  primaryLight: string; // 主色亮
  primaryDark: string;  // 主色暗
  text: string;         // 文字色
...
export function generateACPCard(data: ACPData, options: CardOptions = { theme: 'moltbook' }): string {
  // 解析主题
  ...
  const displayName = data.identity.display_name || name;
  const id = data.identity.id;
  const description = data.identity.description || '';
  const trustLevel = data.capabilities.trust_level || 'unverified';
  const services = data.capabilities.services.map(s => 
    typeof s === 'string' ? s : s.name
  ).slice(0, 3);
  const karma = data.reputation?.karma || 0;
  const followers = data.reputation?.followers || 0;
  const following = data.reputation?.following || 0;
  const owner = data.owners?.[0];
  const homepage = data.entry.homepage || data.entry.source;
  const a2aEnabled = data.a2a?.enabled || false;
  
  // 简化 URL
  const displayUrl = homepage
    .replace('https://', '')
    .replace('http://', '')
    .replace('www.', '');
  
  // 截断描述
  const descLines = splitDescription(description, 35);
  ...
  <g transform="translate(${width - 105}, 75)">
    ...
    <text x="40" y="14" text-anchor="middle" fill="${theme.primary}" font-family="'Courier New', monospace" font-size="10" font-weight="bold">✓ ${trustLevel.toUpperCase()}</text>
  </g>
  ` : ''}
  ...
  <g transform="translate(40, 310)">
    ...
    <g transform="translate(0, 28)">
      ...
      <text x="30" y="16" text-anchor="middle" fill="#ffcc00" font-family="'Courier New', monospace" font-size="14" font-weight="bold">★ ${karma}</text>
      
      <rect x="70" y="0" width="80" height="24" fill="${theme.accent}" opacity="0.2"/>
      <rect x="72" y="2" width="76" height="20" fill="${theme.bgAlt}"/>
      <text x="110" y="16" text-anchor="middle" fill="${theme.accent}" font-family="'Courier New', monospace" font-size="11">${followers} FLWR</text>
      
      <rect x="160" y="0" width="80" height="24" fill="${theme.accent}" opacity="0.2"/>
      <rect x="162" y="2" width="76" height="20" fill="${theme.bgAlt}"/>
      <text x="200" y="16" text-anchor="middle" fill="${theme.accent}" font-family="'Courier New', monospace" font-size="11">${following} FLLW</text>
    </g>
  </g>
  ...
  <g transform="translate(40, 465)">
    ...
    <g transform="translate(0, 25)">
      ...
      <text x="10" y="15" fill="${theme.text}" font-family="'Courier New', monospace" font-size="11">☻ ${owner.name}</text>
      <text x="105" y="15" fill="${theme.accent}" font-family="'Courier New', monospace" font-size="10">${owner.handle || ''}</text>
      ${owner.verification?.claim_verified ? `
  ...
  <g transform="translate(40, 515)">
    ...
    ${a2aEnabled && options.showA2A !== false ? `
...
export { THEMES, ThemeColors, ACPData, CardOptions };
...
// 生成不同主题的卡片
const moltbookCard = generateACPCard(acpData, { theme: 'moltbook' });
const matrixCard = generateACPCard(acpData, { theme: 'matrix' });
const vaporwaveCard = generateACPCard(acpData, { theme: 'vaporwave' });

// 保存为文件
fs.writeFileSync('card-moltbook.svg', moltbookCard);
fs.writeFileSync('card-matrix.svg', matrixCard);
fs.writeFileSync('card-vaporwave.svg', vaporwaveCard);
*/
