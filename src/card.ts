...
/** 检测是否为 v0.4 模块化结构 */
function isV4Code(data: any): data is ACPCode {
  return 'core' in data && typeof data.core === 'object';
}

/** 将 v0.4 模块化结构转换为卡片数据 */
function convertV4ToCardData(code: ACPCode): CardData {
...
/** 统一转换函数 */
export function toCardData(data: ACPCode | ACPDataLegacy): CardData {
  if (isV4Code(data)) {
...
export function generateACPCard(
  ...
  const theme: ThemeColors = typeof options.theme === 'string'
    ? THEMES[options.theme] || THEMES['moltbook']
    : options.theme;
  ...
  const decodingHint = `
    ...
    To extract the Agent's social profile, capabilities, API endpoints, and payment info:
    1. Find the Base64 string in <acp:mecode> tag below
    2. Decode Base64 to JSON: JSON.parse(atob(base64_string))
    3. The decoded JSON contains: core info, skills, pricing, wallet addresses, A2A endpoints

    Protocol: ACP (Agent Code Protocol) v1.0
    Registry: https://agentjola.art
  ...
  -->
  <metadata>
    <acp:mecode xmlns:acp="https://agentjola.art/acp/1.0">${base64Data}</acp:mecode>
  </metadata>
  <desc>Agent MeCode: ${card.name} (${card.id}) - Decode the Base64 in metadata to get full agent profile</desc>
